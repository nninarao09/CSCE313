...
int ackerman(int a, int b) {
    ...
    int val = rand() % 128;
	//cout << "val " << val << endl;
	//cout << "to alloc " << to_alloc << endl;
    
    for(int i = 0; i < to_alloc; i++) {
		//cout << "hellolelo" << endl;
        test_array[i] = val;
		//cout<< "Test array: " << test_array[i]<< endl;
		
    }
	//cout << "jkehgfkjeygfrefr" << endl;
...
int main(int argc, char * argv[]) {
	...
	while((option = getopt(argc, argv, "s:b:")) != -1){
		switch(option){
			case 's': 
				cout << "s = " << atoi(optarg) << endl;
				break;
			case 'b':
				cout << "b = " << atoi(optarg) << endl;
				break;
			case'?':
